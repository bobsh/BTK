...
function Raven:Client(dsn, config)
	...
	client.authHeader = (
		"Sentry sentry_version=%d,sentry_timestamp=%s,sentry_key=%s,sentry_secret=%s,sentry_client=%s"
	):format(
		sentryVersion,
		"%s",
		publicKey,
...
local function ScrubData(playerName, errorMessage, traceback)
	...
	if (traceback ~= nil) then
		...
		if (success) then
			for _, frame in pairs(stacktrace) do
...
function Raven:ConnectRemoteEvent(remoteEvent)
	assert(typeof(remoteEvent) == "Instance" and
		remoteEvent.ClassName == "RemoteEvent",
		"ConnectRemoteEvent did not receive RemoteEvent instance")
		...
		if (count > 0) then
			if (type(errorMessage) == "string" and (type(traceback) == "string" or traceback == nil)) then
				...
				if (success) then
					...
					if (logWarnings) then
						warn(("Raven: Player '%s' tried to send spoofed data, their ability to report errors has been disabled."):format(
							player.Name
						))
						warn("errorMessage:")
						warn(errorMessage)
