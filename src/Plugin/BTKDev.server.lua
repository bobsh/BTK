...
function Assets:InstallAsset(asset)
	...
	print(("Assets: Done installing %s with id %d version %d into %s"):format(asset.Name, asset.ID, versionID, asset.Parent.Name))
end
...
function DevToolBar:Init()
	...
	diagsButton.Click:connect(function()
		...
		for idx, value in pairs(coreGUIChildren) do
	...
	updateButtons()
	btkMod.Parent.ChildAdded:connect(function(child)
		updateButtons()
	end)
	btkMod.Parent.ChildRemoved:connect(function(child)
		updateButtons()
	end)
	btkContent.Parent.ChildAdded:connect(function(child)
		updateButtons()
	end)
	btkContent.Parent.ChildRemoved:connect(function(child)
		updateButtons()
	end)
